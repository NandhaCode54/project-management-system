...
- Scoped, secure **Projects** and **Tasks** CRUD — a user can only ever see/touch their own data.
...
│   └── src/
│       ├── config/             # env, db singleton, logger
│       ├── controllers/        # request/response handling only
│       ├── middleware/         # auth, validate, errorHandler, rate limit, 404
│       ├── routes/             # wiring controllers -> middlewares
│       ├── services/           # business logic -> Prisma
│       ├── validators/         # zod schemas
│       ├── utils/              # ApiError, asyncHandler, jwt helpers, prisma toDto
...
Backend layering (strict): **Routes → Controllers → Services → Prisma**. Controllers never call Prisma; services never touch `req`/`res`.

## 4. Data Model

**User** — id (cuid), fullName, email (unique), passwordHash, createdAt, updatedAt.
**Project** — id, userId (FK→User, cascade), name, description, status enum (NOT_STARTED | IN_PROGRESS | COMPLETED), startDate, endDate, createdAt, updatedAt.
**Task** — id, projectId (FK→Project, cascade => deleting a project cascades its tasks), name, description, priority enum (LOW | MEDIUM | HIGH), status enum (PENDING | IN_PROGRESS | COMPLETED), dueDate, createdAt, updatedAt.
...
- Cookie strategy: `token` HttpOnly, `Secure` (prod), `SameSite=Lax`. Logout clears the cookie. The frontend also keeps nothing sensitive in localStorage (only a non-sensitive `pms_user` convenience mirror — documented trade-off; token is cookie-only).
...
- helmet (security headers), CORS restricted to configured origins, morgan/pino request logging, centralized error handler (single zod error mapping → 400, known errors → mapped codes, Prisma P2002 → 409, fallback → 500, no stack leaks to client).
...
- Verify: app boots, `GET /api/health` returns ok, unknown routes → 404 json, zod errors → 400.
...
### Phase 5 — Projects / Tasks / Dashboard
...
- `pages/DashboardPage` — stat cards + Recharts (task status pie, project status pie, task priority bar).
...
- Landscape verified with a manual smoke run (register → create project → create task → dashboard counts → logout).